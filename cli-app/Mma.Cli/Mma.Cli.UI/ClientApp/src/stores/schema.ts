...
export const useSchemaStore = defineStore('schema', () => {
  ...
  watch([nodes, edges, solutionName, mapper], () => {
    if (autoSave.value) {
      saveSchema()
    }
  }, { deep: true })
  ...
  async function browseDirectories(path?: string) {
    const url = path ? `/api/schema/project/directories?path=${encodeURIComponent(path)}` : '/api/schema/project/directories'
  ...
  async function loadSchema() {
    try {
      const url = cwd.value ? `/api/schema/load?path=${encodeURIComponent(cwd.value)}` : '/api/schema/load'
  ...
  async function importSelected() {
    const res = await fetch('/api/schema/database/import', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...dbConfig.value, tableNames: selectedTables.value })
    })
    if (res.ok) {
      const data = await res.json()
      const newNodes: Node[] = []
